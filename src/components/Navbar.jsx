        ...
        <>
            <div className="w-full flex justify-between items-center font-semibold">
                <div className="flex items-center gap-2">
                    <img className={`w-8 ${location.pathname !== '/' ? 'bg-gray-950' : 'bg-gray-900'} p-2 rounded-2xl ${location.pathname !== '/' ? 'cursor-pointer' : 'cursor-not-allowed'}`} src={assets.arrow_left} alt="" onClick={() => navigate(-1)} />
                    <img className={`w-8  ${location.pathname === '/' ? 'bg-gray-950' : 'bg-gray-900'} p-2 rounded-2xl  ${location.pathname === '/' ? 'cursor-pointer' : 'cursor-not-allowed'}`} src={assets.arrow_right} alt="" onClick={() => navigate(+1)} />
                </div>
